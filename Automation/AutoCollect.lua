return function(SubTab, Window)
    ...
    getgenv().TakeGems = getgenv().TakeGems or true  -- Toggle Gems
    ...
    local MultiDrop
    MultiDrop = SubTab:AddMultiDropdown("Blacklist Items", currentPool, function(selected) 
    ...
    SubTab:AddButton("Scan World Items", function()
        ...
        Window:Notify("Scan complete!", 2)
    end)

    -- RESET FILTER PERBAIKAN
    SubTab:AddButton("Reset All Filters", function()
        getgenv().ItemBlacklist = {}
        badItems = {}
        lockedDoors = {}
        FilterLabel:SetText("Active Blacklist: None")
        MultiDrop:Set({}) -- Membersihkan pilihan di dropdown
        Window:Notify("Filters & Dropdown cleared!", 2)
    end)
                ...
                local success, err = pcall(function()
                    ...
                    if Hitbox and target then
                        ...
                        if path then
                            ...
                            -- PERGERAKAN DENGAN UPDATE STATUS TIAP LANGKAH
                            for i, point in ipairs(path) do
                                if not getgenv().AutoCollect then break end
                                
                                StatusLabel:SetText("Status: " .. (terpaksa and "⚠️ FORCED" or "✅ SAFE") .. " (" .. i .. "/" .. #path .. ")")
