return function(SubTab, Window)
    ...
    getgenv().TakeGems = (getgenv().TakeGems == nil) and true or getgenv().TakeGems
    ...
    SubTab:AddInput("Avoidance Radius (Cost)", tostring(getgenv().AvoidanceStrength), function(v)
        ...
        if val then
            ...
            if Window and Window.Notify then Window:Notify("Avoidance set to: " .. val, 2, "info") end
        end
    end)
    ...
    SubTab:AddButton("Scan World Items", function()
        ...
        currentPool = found
        if MultiDrop and MultiDrop.UpdateList then MultiDrop:UpdateList(found) end
        if Window and Window.Notify then Window:Notify("Found " .. #found .. " item types.", 2, "ok") end
    end)
    ...
    SubTab:AddButton("Reset All Filters", function()
        ...
        if MultiDrop and MultiDrop.ClearAll then MultiDrop:ClearAll() end
        if Window and Window.Notify then Window:Notify("All settings cleared!", 2, "ok") end
    end)
