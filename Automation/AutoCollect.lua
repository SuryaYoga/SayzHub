...
local function findSmartPath(startX, startY, targetX, targetY)
    ...
    -- MULTIDROP_DOWN DENGAN HANDLE UPDATE
    local MultiDrop = SubTab:AddMultiDropdown("Select Items to Block", currentPool, function(selectedTable)
        getgenv().ItemBlacklist = selectedTable
        local count = 0
        for _ in pairs(selectedTable) do count = count + 1 end
        FilterLabel:SetText(count == 0 and "🚫 Blacklist: None" or "🚫 Blacklist: " .. count .. " items")
    end)
